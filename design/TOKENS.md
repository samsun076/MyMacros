...
| `--status-scrim` | The top counterpart to `--chrome`: what scrolled content passes **under** at the safe area (#93). `black-translucent` + `viewport-fit=cover` run the page to y=0 on purpose, so without this a scrolled earned stamp draws across the clock and the battery. Defined once as `var(--bg-top)` — the canvas colour on phone widths, so each pack inherits its own paper and there is no second copy to keep in step. Named because matching the canvas is a *decision*: `black-translucent` paints the status-bar glyphs **white**, so a light pack may need a dark plate here instead. **Open for #30 and answerable only on a device** — headless Chrome reports the inset as 0 and renders the scrim at zero height |
